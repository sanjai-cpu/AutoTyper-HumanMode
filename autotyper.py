...
class StealthGUIApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Stealth Auto Typer - Human Mode (No Repeats)")
    ...
    def setup_gui(self):
        tk.Label(self.root, text="Stealth Auto Typer (Human Mode - No Repeats)",
                 font=("Helvetica", 16, "bold"), bg="#121212", fg="#76FF03").pack(pady=10)
        tk.Button(self.root, text="Select File", command=self.select_file,
                  bg="#76FF03", fg="black").pack(pady=5)
        ...
        self.countdown_label = tk.Label(self.root, text="", bg="#121212",
                                        fg="#FFD700", font=("Courier", 12, "bold"))
        ...
        tk.Button(self.root, text="Start Typing", command=self.start_typing_thread,
                  bg="#2979FF", fg="white").pack(pady=10)
        tk.Button(self.root, text="Stop", command=self.stop_typing,
                  bg="#FF1744", fg="white").pack(pady=5)

        self.char_timer_label = tk.Label(self.root,
                                         text="Characters typed: 0 | Time: 00:00",
                                         bg="#121212", fg="white")
        self.char_timer_label.pack(pady=10)

        tk.Label(self.root, text="Hotkeys:  P = Pause/Resume | Ctrl+Q = Quit",
                 bg="#121212", fg="#FFD700", font=("Consolas", 10)).pack(pady=10)
    ...
    def start_typing_thread(self):
        ...
        if not self.file_path or not self.lines:
            ...
            return
        threading.Thread(target=self.start_typing, args=(delay,), daemon=True).start()
    ...
    def start_typing(self, delay):
        ...
        self.countdown_label.config(text="Typing started...")
        ...
        for line in self.lines:
            ...
            while i < len(char_buffer):
                ...
                # 🔸 Pause logic
                while self.pause_typing and self.typing_active:
                    self.status.set("Paused (Press P to Resume)")
                    time.sleep(0.3)
                ...
                if psutil.cpu_percent() > 85:
                    self.status.set("CPU busy, waiting...")
                ...
                self.char_timer_label.config(
                    text=f"Characters typed: {self.typed_chars} | Time: {mins:02}:{secs:02}"
                )
                i += 1
    ...
    def toggle_pause(self):
        if self.typing_active:
            ...
            if self.pause_typing:
                ...
                self.status.set("Resuming typing...")
    ...
    def force_quit(self):
        self.typing_active = False
        self.root.destroy()
        os._exit(0)  # 💀 Force quit instantly
